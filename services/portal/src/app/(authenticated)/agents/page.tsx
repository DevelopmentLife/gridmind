...
const MOCK_AGENTS: Agent[] = [
  ...
  { agentId: "oracle-1", agentName: "oracle", displayName: "ORACLE", description: "Query intelligence — identifies slow queries, regressions, and optimization opportunities", tier: "reasoning", model: "claude-sonnet-4-6", status: "processing", uptimeSeconds: 172800, tasksInFlight: 2, tasksCompleted: 1205, lastActionAt: new Date(Date.now() - 30 * 1000).toISOString(), deploymentId: "deploy-001" },
  ...
  { agentId: "forge-1", agentName: "forge", displayName: "FORGE", description: "Index advisor — recommends and creates indexes to improve query performance", tier: "execution", model: "claude-sonnet-4-6", status: "healthy", uptimeSeconds: 172800, tasksInFlight: 0, tasksCompleted: 89, lastActionAt: new Date(Date.now() - 18 * 60 * 1000).toISOString(), deploymentId: "deploy-001" },
  { agentId: "convoy-1", agentName: "convoy", displayName: "CONVOY", description: "Backup manager — schedules, executes, and verifies database backups", tier: "execution", model: "deterministic", status: "healthy", uptimeSeconds: 172800, tasksInFlight: 0, tasksCompleted: 364, lastActionAt: new Date(Date.now() - 35 * 60 * 1000).toISOString(), deploymentId: "deploy-001" },
  ...
  { agentId: "tuner-1", agentName: "tuner", displayName: "TUNER", description: "Performance tuner — adjusts PostgreSQL configuration parameters for optimal performance", tier: "execution", model: "claude-sonnet-4-6", status: "idle", uptimeSeconds: 172800, tasksInFlight: 0, tasksCompleted: 7, lastActionAt: new Date(Date.now() - 6 * 60 * 60 * 1000).toISOString(), deploymentId: "deploy-001" },
...
const MOCK_ACTIVITY: AgentActivity[] = [
  { activityId: "a1", agentName: "oracle", displayName: "ORACLE", action: "Analyzing slow query log", details: "Found 3 queries with sequential scans > 100ms — generating recommendations", severity: "info", timestamp: new Date(Date.now() - 30 * 1000).toISOString(), deploymentId: "deploy-001", deploymentName: "production-primary" },
...
export default function AgentsPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {!isLoading && agents.length === 0 && (
        <EmptyState title="No agents active" message="Deploy a database to activate your agent fleet." icon="🤖" />
      )}
